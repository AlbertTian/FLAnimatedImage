...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
        ...
        NSNumber *loopCount = [[imageProperties objectForKey:(id)kCGImagePropertyGIFDictionary] objectForKey:(id)kCGImagePropertyGIFLoopCount];
        if (loopCount == nil) {
            _loopCount = 1;
        } else if (loopCount.unsignedIntegerValue > 0) {
